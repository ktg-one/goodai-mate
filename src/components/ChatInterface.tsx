...
// ⚡ Bolt Optimization: Extract input into a memoized component to prevent
// the entire ChatInterface (and message list) from re-rendering on every keystroke.
// Expected Impact: Reduces re-renders from O(keystrokes) to O(messages).
const ChatInput = React.memo(function ChatInput({ isBusy, onSubmit }: ChatInputProps) {
  ...
  return (
    <section className="min-h-[620px] overflow-hidden rounded-[24px] border-2 border-[var(--ink)] bg-white text-[var(--ink)] shadow-[4px_4px_0_var(--ink)]">
      ...
      <div className="flex h-[490px] flex-col bg-[rgba(248,248,246,0.58)]">
        ...
        <ChatInput isBusy={isBusy} onSubmit={handleChatSubmit} />
      </div>
    </section>
  );
}
