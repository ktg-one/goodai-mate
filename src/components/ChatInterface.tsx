...
// ⚡ Bolt: Extracting the input form to prevent the entire message list from re-rendering on every keystroke.
const ChatInputForm = memo(function ChatInputForm({ isBusy, onSubmit }: ChatInputFormProps) {
  ...
  return (
    <section className="min-h-[620px] overflow-hidden rounded-[24px] border-2 border-[var(--ink)] bg-white text-[var(--ink)] shadow-[4px_4px_0_var(--ink)]">
      ...
      <div className="flex h-[490px] flex-col bg-[rgba(248,248,246,0.58)]">
        ...
        <ChatInputForm isBusy={isBusy} onSubmit={handleInputSubmit} />
      </div>
    </section>
  );
}
