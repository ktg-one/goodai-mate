...
// Extracted the input form and its state (`input`, `setInput`) into a separate `ChatInputForm` component.
// This prevents the entire `ChatInterface` (which includes the message history, TTS, and LeadCaptureCard)
// from re-rendering on every single keystroke.
function ChatInputForm({ isBusy, onSend, initialInput = '' }: { isBusy: boolean, onSend: (text: string) => void, initialInput?: string }) {
  ...
  return (
    <section className="min-h-[620px] overflow-hidden rounded-[24px] border-2 border-[var(--ink)] bg-white text-[var(--ink)] shadow-[4px_4px_0_var(--ink)]">
      ...
      <div className="flex h-[490px] flex-col bg-[rgba(248,248,246,0.58)]">
        ...
        <ChatInputForm isBusy={isBusy} onSend={handleSend} />
      </div>
    </section>
  );
}
