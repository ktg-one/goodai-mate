...
async function runGwsCommand(
  ...
  try {
    ...
    console.error(
      `GWS execution failed: ${command} ${fullArgs.map((a) => `"${a}"`).join(" ")}`,
      err,
    );
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (actions.sheet) {
      ...
      const sheetResult = (await runGwsCommand(
      ...
      if (sheetResult && sheetResult.spreadsheetId) {
        ...
        logs.push(
          "[SIMULATED] Sheets: Created spreadsheet and appended lead (GWS CLI not available)",
        );
    ...
    if (actions.doc) {
      ...
      if (docResult && docResult.documentId) {
        ...
        logs.push(
          `[SIMULATED] Docs: Created scope document for ${name} (GWS CLI not available)`,
        );
    ...
    if (actions.emailNotification && email) {
      ...
      if (emailResult && emailResult.id) {
        ...
        logs.push(
          `Email notification sent successfully! ID: ${emailResult.id}`,
        );
      } else {
        logs.push(
          `[SIMULATED] Gmail: Sent email notification to ${email} (GWS CLI not available)`,
        );
    ...
    if (actions.calendar) {
      ...
      if (eventResult && eventResult.htmlLink) {
        ...
        logs.push(
          `[SIMULATED] Calendar: Scheduled discovery call (GWS CLI not available)`,
        );
    ...
    if (actions.n8n) {
      ...
      if (trimmedUrl && !(await isSafeUrl(trimmedUrl))) {
        logs.push(`[ERROR] Blocked unsafe webhook URL: ${trimmedUrl}`);
        results.n8nStatus = "Blocked (Invalid URL)";
      } else {
        const targetUrl =
          trimmedUrl ||
          process.env.N8N_DEMO_WEBHOOK_URL ||
          process.env.N8N_CALL_WEBHOOK_URL ||
          "http://localhost:5678/webhook/goodai-demo";
        ...
        try {
          ...
          if (n8nRes.ok) {
            logs.push(
              `[SYSTEM] Webhook trigger success! Response: ${n8nRes.status} ${n8nRes.statusText}`,
            );
            results.n8nStatus = "Success";
          } else {
            logs.push(
              `[WARNING] Webhook URL returned status ${n8nRes.status}. Check your n8n workflow.`,
            );
          ...
          if (errorObject.name === "AbortError") {
            logs.push(
              "[WARNING] Webhook connection timed out. If you do not have n8n running locally, this is expected.",
            );
          } else {
            logs.push(
              `[WARNING] Webhook trigger failed: ${errorObject.message || "Unknown error"}. (Using simulated webhook demo fallback)`,
            );
          }
