...
## 2026-07-15 - Added ARIA Announcements to ChatInterface
**Learning:** Vercel AI SDK chat implementations often render incoming streaming messages dynamically. Screen readers do not announce these new messages if they are just appended to the DOM without explicit roles and live regions.
**Action:** Always wrap dynamic chat thread containers with `role="log"` and `aria-live="polite"`. Additionally, explicitly declare `role="status"` on typing indicators and `role="alert" aria-live="assertive"` on error messages to ensure critical conversational context is announced immediately.
