...
## 2026-07-23 - Improve screen reader experience for ChatInterface
**Learning:** Dynamic chat threads using the Vercel AI SDK append streaming messages and status updates (typing/errors) dynamically. Without explicit roles and live regions, screen readers may not announce these updates.
**Action:** Always wrap the chat thread container with `role="log"` and `aria-live="polite"`. Apply `role="status"` to typing indicators and `role="alert" aria-live="assertive"` to error messages to ensure timely announcements.
