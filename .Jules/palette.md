...
## 2026-07-20 - Add ARIA live regions to chat interfaces
**Learning:** Chat interfaces with dynamically streaming or appended messages are not announced to screen readers by default.
**Action:** Always wrap chat thread containers with `role="log"` and `aria-live="polite"`. Use `role="status"` for typing indicators and `role="alert" aria-live="assertive"` for error messages.
