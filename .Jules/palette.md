...
## 2024-05-19 - Add ARIA live regions to chat interfaces
**Learning:** For chat interfaces (like Vercel AI SDK wrappers) where messages stream in dynamically, screen readers won't announce the new content unless the container uses `role="log"` and `aria-live="polite"`. Additionally, typing indicators should use `role="status" aria-live="polite"` and error messages should use `role="alert" aria-live="assertive"` to ensure users know what's happening.
**Action:** Always add `role="log"` and `aria-live="polite"` to dynamically updating conversational threads. Use `role="status"` and `role="alert"` for transient UI feedback within these contexts.
