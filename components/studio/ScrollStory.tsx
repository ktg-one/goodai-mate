...
export function ScrollStory() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      media.add("(min-width: 761px) and (prefers-reduced-motion: no-preference)", () => {
        ...
        const chapterItems = scope.querySelectorAll<HTMLElement>(".story-chapter-item");
        ...
        gsap.set(chapterItems, { opacity: 0, y: 30 });
        if (scenes[0]) gsap.set(scenes[0], { opacity: 1 });
        if (chapterItems[0]) gsap.set(chapterItems[0], { opacity: 1, y: 0 });

        const paths0 = scenes[0]?.querySelectorAll<SVGPathElement>("[data-ink]") || [];

        // Master pinned timeline across ~3200px scroll track
        ...
        // Chapter 1 drawing
        mainTl.to(paths0, { strokeDashoffset: 0, duration: 1, stagger: 0.04, ease: "none" })
              .to({}, { duration: 0.8 }); // Reading hold for Ch 1

        // Transition Ch 1 -> Ch 2
        const paths1 = scenes[1]?.querySelectorAll<SVGPathElement>("[data-ink]") || [];
        mainTl.to(chapterItems[0], { opacity: 0, y: -20, duration: 0.5, ease: "power1.inOut" }, ">")
              .to(scenes[0], { opacity: 0, duration: 0.5, ease: "power1.inOut" }, "<")
              .to(chapterItems[1], { opacity: 1, y: 0, duration: 0.5, ease: "power1.inOut" }, ">-0.2")
              .to(scenes[1], { opacity: 1, duration: 0.5, ease: "power1.inOut" }, "<")
              .to(paths1, { strokeDashoffset: 0, duration: 1, stagger: 0.04, ease: "none" }, ">-0.1")
              .to({}, { duration: 0.8 }); // Reading hold for Ch 2

        // Transition Ch 2 -> Ch 3
        const paths2 = scenes[2]?.querySelectorAll<SVGPathElement>("[data-ink]") || [];
        mainTl.to(chapterItems[1], { opacity: 0, y: -20, duration: 0.5, ease: "power1.inOut" }, ">")
              .to(scenes[1], { opacity: 0, duration: 0.5, ease: "power1.inOut" }, "<")
              .to(chapterItems[2], { opacity: 1, y: 0, duration: 0.5, ease: "power1.inOut" }, ">-0.2")
              .to(scenes[2], { opacity: 1, duration: 0.5, ease: "power1.inOut" }, "<")
              .to(paths2, { strokeDashoffset: 0, duration: 1, stagger: 0.04, ease: "none" }, ">-0.1")
              .to({}, { duration: 1 }); // Final reading hold for Ch 3
      });
  ...
  return (
    <section className="scroll-story shell" ref={root} id="features" aria-labelledby="scroll-story-title">
      <div className="story-intro">
        <h2 id="scroll-story-title">You’ve done enough<br /><em>for one day.</em></h2>
        <p>Let’s get the work out of the way.</p>
      </div>
      <div className="story-layout">
        <div className="story-chapters-pinned">
          {chapters.map((chapter, index) => (
            <article className="story-chapter-item" key={chapter.title}>
              <div className="chapter-copy">
                <h3>{chapter.title}</h3>
                <p>{chapter.copy}</p>
                <span className="handwritten chapter-note">{chapter.note}</span>
              </div>
              <div className="story-mobile-art" aria-hidden="true">
                <StoryDrawing scene={index}/>
                <span className="drawing-caption">{chapter.caption}</span>
              </div>
            </article>
          ))}
        </div>
        <div className="story-canvas" aria-hidden="true">
          {chapters.map((chapter, index) => (
            <div data-scene={index} key={chapter.title}>
              <StoryDrawing scene={index}/>
              <span className="drawing-caption">{chapter.caption}</span>
            </div>
          ))}
      ...
      <div className="story-footer-link">
        <a href="#services" className="text-link story-next">
...
function StoryDrawing({scene}:{scene:number}) {
  return (
    <svg className="story-drawing" viewBox="0 0 560 440" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      {scene === 0 && (
        <>
