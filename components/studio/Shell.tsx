...
export function StudioHeader() {
  ...
  return <header className="site-header"><div className="shell header-inner"><Link className="brand-link" href="/" aria-label="Good'Ai home" onClick={() => setOpen(false)}><Brand /></Link><nav className="desktop-nav" aria-label="Main navigation"><Link href="/#services">What we do</Link><Link href="/#approach">How we work</Link><a href={PHONE_HREF} className="header-phone" aria-label={`Call our AI voice agent on ${PHONE_DISPLAY}`}>{PHONE_DISPLAY}<span>Call our AI agent</span></a></nav><a href={SURVEY_URL} target="_blank" rel="noopener noreferrer" className="button button-small header-cta">Let’s talk <ArrowUpRight size={17} /></a><button ref={menu} type="button" className="menu-toggle" aria-expanded={open} aria-controls="mobile-nav" aria-label={open ? "Close menu" : "Open menu"} onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button></div>{open && <nav id="mobile-nav" className="mobile-nav shell" aria-label="Mobile navigation"><Link href="/#services" onClick={() => setOpen(false)}>What we do <ArrowRight /></Link><Link href="/#approach" onClick={() => setOpen(false)}>How we work <ArrowRight /></Link><Link href="/#voice" onClick={() => setOpen(false)}>Try the voice agent <ArrowRight /></Link><a href={PHONE_HREF} onClick={() => setOpen(false)}>Call our AI agent: {PHONE_DISPLAY} <ArrowRight /></a><Link href="/demo" onClick={() => setOpen(false)}>Workflow examples <ArrowRight /></Link><a href={SURVEY_URL} target="_blank" rel="noopener noreferrer">Tell us what’s eating your week <ArrowUpRight /></a></nav>}</header>;
}
export function StudioFooter() {
  return <footer className="site-footer"><div className="shell"><div className="footer-top"><p>Built in Perth.<br />Good wherever you work.<br /><a href={PHONE_HREF} className="footer-phone">{PHONE_DISPLAY}</a><br /><span className="phone-caption">Business line / AI voice agent</span></p><nav aria-label="Footer navigation"><Link href="/#services">Services</Link><Link href="/#approach">Our approach</Link><a href="https://goodai.up.railway.app/" target="_blank" rel="noopener noreferrer">Field notes <ArrowUpRight size={14} /></a><a href={SURVEY_URL} target="_blank" rel="noopener noreferrer">Get in touch <ArrowUpRight size={14} /></a></nav></div><Link href="/" className="footer-wordmark" aria-label="Good'Ai home"><Brand large /></Link><div className="footer-bottom"><span>© {new Date().getFullYear()} Good’Ai</span><span>Less busywork. More living.</span><div><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></div></div></div></footer>;
}
