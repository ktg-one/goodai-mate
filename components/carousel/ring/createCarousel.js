...
// Owns one mounted WebGL scene. Call the returned disposer on unmount.
export function createCarousel({ containerRef, listRef, itemsRef, loaderRef, liveRef, cutRef, metaRef }, hero = false) {
  const container = containerRef.current;
  const listEl = listRef.current;
  const loaderEl = loaderRef.current;
  // Async atlas work can land after cleanup under StrictMode's double
  // mount. Everything deferred checks this.
  let disposed = false;

  const params = defaultParams();
  if (hero) {
    // The hero has its own contained stage: show the whole ring rather than
    // the full-screen layout's large arc cropped beyond the left edge.
    Object.assign(params, {
      refWidth: 1100, refHeight: 700,
      minScale: 0.42, maxScale: 0.8, fitHeight: 1,
      ringRadius: 215, planeSize: 100,
      endScale: 1.4, narrowEndScale: 1.4,
      posX: 0, narrowPosX: 0, tightPosX: 0,
      narrowRadius: 1, tightRadius: 1, narrowPlane: 1,
      narrowText: 1, tightName: 1, nameSize: 3.8,
      tightMetaWidth: 90, tightNameBottom: 0,
      launchTime: 0.9, spreadTime: 1.5, spinTime: 1.2, moveTime: 1.2,
      glass: false,
    });
  }
  ...
  const uniforms = {
    ...
    uTime: { value: 0 },
    uColor: { value: new THREE.Color("#202B3D") },
    ...
    uPage: { value: new THREE.Color("#FFF0D0") },
  };
  ...
  const styleMeta = () =>
    meta.style({
      textK: hero ? Math.max(textK, 18 / (viewW * params.nameSize / 100)) : textK,
      tight: hero || tightNow,
      viewW,
    });
  ...
  const onKeyDown = (e) => {
    ...
    spinVel = 0;
    state.spin +=
      e.key === "ArrowRight" ? -TAU / params.count : TAU / params.count;
    announced = -1;
  };
