...
export function buildAtlas(files = IMAGE_FILES, onProgress) {
  ...
  const paint = (img, i) => {
    ...
    const isScreenshot = /\.png$/i.test(files[i]);

    // Screenshots need their full interface visible. Artwork can still fill
    // the frame, but product proof is contained rather than cropped.
    const scale = isScreenshot
      ? Math.min(CELL_W / img.width, CELL_H / img.height)
