...
export async function generateMetadata({params}: {params:Promise<{slug:string}>}) { const {slug}=await params; const service=services.find(item=>item.slug===slug); return {title:service?.name ?? "Service not found", description:service?.description}; }
