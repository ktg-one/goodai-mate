...
## 2026-06-05 - [Vercel AI SDK Lazy Evaluated Derived State]
**Learning:** In chat interfaces using the Vercel AI SDK, computing derived state (like string concatenation via `messages.map(...).join()`) inside a `useMemo` solely dependent on `messages` causes O(N) operations per streaming token.
**Action:** Always lazy-evaluate expensive derived state in `useMemo` by checking component visibility or utilization state (e.g. `showLeadCard && !leadDismissed`) to prevent high CPU usage during streaming token updates.
