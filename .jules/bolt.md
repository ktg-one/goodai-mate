...
## 2024-05-19 - Fix RAF burn on paused visualizer components
**Learning:** Found two visualizer components (`Visualizer.tsx`, `AudioVisualizer.tsx`) that continuously polled `requestAnimationFrame(render)` even when they were paused (e.g. offscreen or when tab was hidden) because the components were not fully halting execution of the loop despite doing no rendering work.
**Action:** When pausing a `requestAnimationFrame` loop, completely remove the call to schedule the next frame. Then, use event listeners (like `IntersectionObserver` or `visibilitychange`) to detect when the component is unpaused and call the `render` function directly to restart the loop safely.
