...
## 2024-08-16 - Prevent Spreadsheet Formula Injection (CSV Injection) in Demo Automation
**Vulnerability:** The `/api/demo-automation` endpoint appended unsanitized user inputs (name, business, phone, email, problem) to a Google Spreadsheet using `valueInputOption: 'USER_ENTERED'`. This allowed attackers to inject spreadsheet formulas (CWE-1236) by starting their input with `=`, `+`, `-`, or `@`, potentially exfiltrating data or executing malicious code when the sheet is opened.
**Learning:** Using `valueInputOption: 'USER_ENTERED'` forces Google Sheets to interpret the input as if a user typed it in the UI, parsing formulas automatically. When taking untrusted input from the public web, this creates an injection vector if the input is not strictly validated or escaped as plain text.
**Prevention:** To prevent CSV/Spreadsheet Formula Injection, always sanitize user input by prefixing any cell value that starts with a formula trigger character (`=`, `+`, `-`, `@`, `\t`, `\r`) with a single quote (`'`), which instructs Google Sheets to treat the content strictly as text, neutralizing the formula evaluation.
