...
## 2024-05-24 - SSRF Filter Bypass via IPv4-Mapped IPv6 Obfuscation
**Vulnerability:** The SSRF protection utility `isSafeUrl` incorrectly parsed IPv4-mapped IPv6 addresses by strictly matching the prefix string `::ffff:`. Attackers could bypass this check by providing uncompressed or alternatively formatted IPv6 addresses (e.g., `0:0:0:0:0:ffff:127.0.0.1` or `[::ffff:7f00:1]`) that evade the prefix match but still resolve back to a private IPv4 local interface during `fetch` requests.
**Learning:** `net.isIPv6()` returns true for IPv4-mapped IPv6 addresses, but Node's `dns.promises.lookup` and `URL` normalize and return these values differently based on the input format and system resolver. Strict string-matching (like `.startsWith('::ffff:')`) is extremely brittle and insecure when parsing complex protocols like IPv6 where zero-compression is optional.
**Prevention:** Instead of naive string matching, securely tokenize the IPv6 string using its separator (`:`). Ensure that all preceding blocks logically evaluate to exactly zero (`0`, `0000`, or empty) and that the immediate preceding block is exactly `ffff`. This provides a robust extraction of the encapsulated IPv4 segment for downstream evaluation.
