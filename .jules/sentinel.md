...
## 2025-02-14 - SSRF Bypass via Unspecified IPv6 Addresses (::)
**Vulnerability:** The SSRF validation utility `isSafeUrl` in `src/lib/ssrf.ts` allowed unspecified IPv6 addresses (e.g., `::`, `0000::`, `0:0:0:0:0:0:0:0`) to bypass validation. Many operating systems resolve these unspecified addresses to the loopback interface (`localhost` / `127.0.0.1` / `::1`), which allowed an attacker to access internal services and bypass the SSRF protection entirely.
**Learning:** `net.isIPv6` returns `true` for unspecified addresses, but the script did not explicitly filter them out, checking only for the strict loopback representation `::1` and specific private prefixes (`fc`, `fd`, `fe80`).
**Prevention:** Always sanitize and block the unspecified IPv6 address variants by stripping all colons, zeros, and dots from the string and checking if the resulting string is empty, or block them explicitly alongside `::1`. Furthermore, comprehensively block all Link-Local (`fe`) and Multicast (`ff`) prefixes rather than just `fe80`.
