...
## 2025-02-14 - SSRF Bypass via non-shorthand IPv4-mapped IPv6 Addresses
**Vulnerability:** The `parseIPv4MappedIPv6` function inside `src/lib/ssrf.ts` correctly parsed shorthanded mapped formats (e.g. `::ffff:127.0.0.1`), but strictly failed to parse valid non-shorthanded representations such as `0:0:0:0:0:ffff:127.0.0.1`. Because `net.isIPv6()` returns true for these valid IPv6 formats, it completely bypassed the restricted loopback validation check when `.startsWith('::ffff:')` evaluated to false. Also, a naive patch extracting any IPv4 suffix creates a new bypass where a private IPv6 (e.g. `fd00::8.8.8.8`) extracts a public IPv4 and gets permitted.
**Learning:** Checking for specific string structures (like `::ffff:`) is an incomplete and dangerous way to identify valid IPv6 formats, as IPv6 zero-compression is optional. Furthermore, blindly extracting an IPv4 suffix without verifying the entire mapped prefix (e.g., ensuring all prior segments evaluate to zero and the immediate preceding segment is `ffff`) opens up SSRF vectors on IPv6 networks.
**Prevention:** Extract the IPv4 portion of an IPv4-mapped IPv6 address only after strictly validating that the preceding segments form a valid mapped prefix mathematically.
