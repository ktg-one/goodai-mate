...
## 2024-05-30 - Fix SSRF Vulnerability via IPv4-mapped IPv6 Address Bypass
**Vulnerability:** The `isSafeUrl` function in `src/lib/ssrf.ts` is vulnerable to SSRF via IPv4-mapped IPv6 addresses when a domain name resolves to one (e.g., `localtest.me` resolving to `::ffff:127.0.0.1` or when provided an IP). The `net.isIPv6` returns true for these addresses, but they bypass the explicit IPv4 checks and are considered "safe" by the current IPv6 logic, allowing attackers to target internal network resources (like localhost or internal IP ranges).
**Learning:** `net.isIPv6()` returns `true` for IPv4-mapped IPv6 addresses, but typical IPv6 validation (checking for `::1`, `fc`, `fd`, `fe80`) fails to catch the embedded IPv4 loopback or private addresses.
**Prevention:** Explicitly check for the `::ffff:` prefix (or use a regex or dedicated utility) in the IPv6 validation block, extract the embedded IPv4 address, and run it through the same IPv4 safety checks.
