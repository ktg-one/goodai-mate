...
## 2025-02-24 - SSRF Bypass via IPv4-Mapped IPv6 Formats
**Vulnerability:** The `parseIPv4MappedIPv6` function in `src/lib/ssrf.ts` incorrectly assumed that all IPv4-mapped IPv6 addresses would strictly begin with `::ffff:`. It failed to handle fully expanded formats (e.g., `0:0:0:0:0:ffff:127.0.0.1`) or alternative zero-compressed forms (e.g., `0::ffff:7f00:1`), allowing attackers to bypass SSRF validation while the underlying network request would resolve to loopback.
**Learning:** Node.js `URL` normalizes IPv4-mapped IPv6 addresses to hex formats (e.g. `[::ffff:7f00:1]`), and native tools accept various valid structural representations of IPv6. Naive string prefix matching (`.startsWith`) is insufficient for IP validation.
**Prevention:** Implement comprehensive IPv6 parsing that accurately expands zero-compression (`::`), counts the structural segments, and strictly validates the 80-bit zero prefix against the `ffff` indicator before extracting the mapped IPv4 payload.
